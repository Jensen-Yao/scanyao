...
import { FilterStrip, FILTERS } from './components/FilterStrip'
...
import { LiveScanBadge } from './components/LiveScanBadge'
...
import { copyPageSettings, fileStem, filterStrengthFor, movePage } from './core/document'
import { composePlacedScans, composeScans } from './core/compositor'
import { DEFAULT_CORNERS, type CornerSet } from './core/geometry'
import { DEFAULT_ADJUSTMENTS, DEFAULT_FILTER_STRENGTHS, detectDocumentCorners, renderScan } from './core/imageEngine'
...
export function App() {
  ...
  // 主预览：增强模式或全屏预览时渲染高清结果；校正模式由实时小卡负责
  useEffect(() => {
    if (!activePage || (mode === 'crop' && !previewOpen)) return
    let disposed = false
  ...
  const openSystemCamera = () => {
    setCameraOpen(false)
    window.setTimeout(() => cameraInput.current?.click(), 0)
  }
  ...
  const exportDecorations = (pageIndex: number) => ({
    watermarkText: exportOptions.watermarkText.trim() || undefined,
    pageNumber: exportOptions.pageNumbers && pages.length > 0
      ? { index: pageIndex + 1, total: pages.length }
      : undefined,
  })
  const renderExportPage = (page: ScanPage, pageIndex: number) => renderScan(
    page.sourceUrl,
    page.corners,
    page.rotation,
    page.filter,
  ...
  const exportJpg = async () => {
    ...
    try {
      ...
      const result = await renderExportPage(activePage, pageIndex)
      await saveBlob(result.blob, `${fileStem(scanDocument.title)}-${String(pageIndex + 1).padStart(2, '0')}.jpg`, '导出扫描 JPG')
      URL.revokeObjectURL(result.url)
      setExportOpen(false)
      setToast('JPG 已准备好')
    } catch {
  ...
  const exportZip = async () => {
    ...
    try {
      ...
      for (let index = 0; index < pages.length; index += 1) {
        ...
        const result = await renderExportPage(pages[index], index)
        entries.push({
  ...
  const exportMerged = async () => {
    ...
    try {
      for (let index = 0; index < pages.length; index += 1) {
        ...
        rendered.push(await renderExportPage(pages[index], index))
      }
  ...
  const activeIndex = pages.findIndex((page) => page.id === selectedId)
  ...
  return (
    <div class="app-shell" onDragOver={(event) => event.preventDefault()} onDrop={onDrop}>
      ...
      <main class="main-content">
        <header class="topbar">
          ...
          <div class="top-actions">
            ...
            <button class="primary-button capture-button" type="button" onClick={openCamera}><Aperture size={17} />拍摄</button>
        ...
        {!activePage ? (
          <section class="empty-state" aria-label="开始扫描">
            ...
            <h2>把纸张，变成清晰的数字文件。</h2>
            <p class="empty-copy">拍摄或导入后自动识别边缘，框选即可实时预览扫描效果。校正、滤镜、拼版与 PDF 导出全部在本机完成。</p>
            <div class="empty-actions"><button class="primary-button large" type="button" onClick={openCamera}><Aperture size={18} />拍一张</button><button class="quiet-button large" type="button" onClick={() => requestImport()}><ImagePlus size={18} />从相册导入</button></div>
              ...
              <div class="inspector-scroll">
                ...
                <section class="control-section">
                  ...
                  <FilterStrip
                    ...
                    onSelect={(filter) => {
                      updateActive({ filter }, true, `切换滤镜：${FILTERS.find((item) => item.id === filter)?.label ?? ''}`)
                      setMode('filter')
                    }}
                  />
      ...
      {cameraOpen && <CameraSheet onFinish={(files) => { setCameraOpen(false); void addFiles(files) }} onClose={() => setCameraOpen(false)} onSystemCamera={openSystemCamera} />}
      {previewOpen && preview && activePage && preview.id === selectedId && (
        <PreviewOverlay
          processedUrl={preview.url}
          originalUrl={activePage.sourceUrl}
          caption={`第 ${activeIndex + 1} / ${pages.length} 页 · ${preview.width} × ${preview.height}`}
          canPrev={activeIndex > 0}
          canNext={activeIndex < pages.length - 1}
          onPrev={() => selectRelativePage(-1)}
          onNext={() => selectRelativePage(1)}
      ...
      {exportOpen && <ExportSheet documentName={scanDocument.title} options={exportOptions} pageCount={pages.length} currentIndex={activeIndex} busy={busy} onNameChange={(title) => updateDocument((current) => ({ ...current, title }))} onOptionsChange={(patch) => setExportOptions((current) => ({ ...current, ...patch }))} onClose={() => setExportOpen(false)} onPdf={() => void exportPdf()} onJpg={() => void exportJpg()} onZip={() => void exportZip()} onMerge={() => void exportMerged()} />}
