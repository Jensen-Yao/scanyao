...
- 应用内相机：实时取景、自动识别文档边缘（绿色高亮）、连拍多页、画面稳定自动拍摄（可选）与手电筒；权限不可用时回退系统相机
- 拍照、相册导入、桌面拖放与多页管理；导入前可选择按页扫描或先拼图
- 独立原图拼合工作台，可继续添加图片、套用长图/横排/双列/**证件正反**/自由模板、拖动位置，并缩放或滑动画布检查细节
- 校正模式右下角**实时扫描预览卡**：框选调整四角、边线或切换滤镜时即时渲染透视校正结果，所见即所得
- **画布缩放**：滚轮或 -/+/适合窗口 控件缩放中间预览（25%–400%，记忆设置）；触屏支持**双指捏合缩放**（以捏合中点为锚）；增强模式以光标为中心缩放，校正模式空白处拖拽平移、增强模式直接拖拽平移
- 全屏预览：滚轮/双指缩放、拖动平移、双击放大、左右翻页、按住**对比原图**
- 自动建议文档边缘；裁剪支持四角 + 四条边中点 + 框内整体移动，触屏拖动时出现**放大镜**辅助对齐，框内带三分构图网格
- 纯 TypeScript 四点透视校正与双线性采样
- 22 种场景滤镜，覆盖印刷文字、阴影、书页、报纸、手写、票据、发票、证件、证书、印章、蓝图、屏幕等扫描场景；滤镜条使用**当前页真实渲染缩略图**
